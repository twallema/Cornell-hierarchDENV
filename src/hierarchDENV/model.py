...
# import C++ model
from hierarchDENV import sir_model
...
class imsSIR():
    ...
    def __init__(self, parameters, initial_condition_function, n_strains):
        ...
        self.initial_states = {'S': np.zeros(n_strains),  'I': np.zeros(n_strains), 'R': np.zeros(n_strains), 'I_inc': np.zeros(n_strains)}
    ...
    def sim(self, simtime, atol=1e-2, rtol=1e-5, N=1, draw_function=None, draw_function_kwargs={}):
        ...
        reference_date = datetime(start_date.year, 10, 1)
        ...
        for _ in range(N):
            ...
            for par, shape in self.parameter_shapes.items():
                if ((shape == (1,)) & (par not in ['T_report', 'rho_report', 'gamma', 'sigma', 'modifier_length']) & (par != 'delta_beta_temporal')):
